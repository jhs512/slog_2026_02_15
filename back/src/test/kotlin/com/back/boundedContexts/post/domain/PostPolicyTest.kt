...
class PostPolicyTest {
    ...
    private val author = Member(1, "user1", "작성자")
    private val admin = Member(2, "admin", "관리자")
    private val other = Member(3, "other", "다른사용자")

    @Nested
    inner class CanRead {
        @Test
        fun `공개 글은 누구나 조회할 수 있다`() {
            val post = Post(author, "제목", "내용", true, true)
        ...
        fun `미공개 글은 작성자만 조회할 수 있다`() {
            val post = Post(author, "제목", "내용", false, false)
